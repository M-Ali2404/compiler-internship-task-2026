...
public class MiniKotlinCompiler : MiniKotlinBaseVisitor<String>() {
    ...
    // a list to keep track of local variables so we know when to add [0]
    private val local_vars = java.util.ArrayList<String>()
    ...
    override fun visitFunctionDeclaration(ctx: MiniKotlinParser.FunctionDeclarationContext): String {
        // clear the local variables list for the new function
        local_vars.clear()
    ...
    private fun process_statements(
        ...
        if (var_decl != null) {
            return translate_expression(var_decl.expression()) { res ->
                ...
                // add it to our list so we know it is an array later
                local_vars.add(var_name)
                
                // FIX: wrap the variable in a 1-element array
                val current_line = var_type + "[] " + var_name + " = new " + var_type + "[]{ " + res + " };\n"
        ...
        if (assign != null) {
            return translate_expression(assign.expression()) { res ->
                ...
                val current_line = var_name + "[0] = " + res + ";\n"
                val rest_of_code = process_statements(tail, cont, ret_type)
    ...
    override fun visitIdentifierExpr(ctx: MiniKotlinParser.IdentifierExprContext): String {
        ...
        // otherwise it is a parameter or function name, just return the name
        return name
    }    
    override fun visitParenExpr(ctx: MiniKotlinParser.ParenExprContext): String {
        return visit(ctx.expression())
    }
    // helper to map minikolin types to java types
